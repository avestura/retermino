...
describe("ReactTerminal", () => {
    ...
    test("down arrow fetch next command", () => {
        ...
        writeText(terminalContainer, "ArrowUp");
        expect(
            terminalContainer.querySelectorAll(".preWhiteSpace")[0].textContent
        ).toBe("");
        expect(
            terminalContainer.querySelectorAll(".preWhiteSpace")[1].textContent
        ).toBe("");
        writeText(terminalContainer, "whoami");
        writeText(terminalContainer, "Enter");
        writeText(terminalContainer, "ArrowUp");
    ...
    test("custom errorMessage is function", async () => {
        render(
            <TerminalContextProvider>
                <ReactTerminal
                  errorMessage={() => "Function but command not found"}
                />
            </TerminalContextProvider>
        );
        ...
        expect(terminalContainer.textContent).toContain(
            "Function but command not found"
        );
    });
    ...
    test("defaultHandler is used if provided when no commands match", async () => {
        ...
        expect(terminalContainer.textContent).toContain(
            "default command handler triggered"
        );
    });
});
